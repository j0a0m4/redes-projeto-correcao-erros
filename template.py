...
for i in range(reps):
    ...
    ##
    insertedErrorCount, transmittedPacket = insertErrors(
        codedPacket, errorProb)
...
print('Numero de bits corrompidos apos decodificacao: {0:d}'.format(
    totalBitErrorCount))
...
print('Taxa de erro de pacotes: {0:.2f}%'.format(
    float(totalPacketErrorCount) / float(reps) * 100.0))
